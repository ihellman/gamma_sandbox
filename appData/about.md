...
* **Germplasm (G):** Location data for living accessions (seeds or plants) held in conservation collections.
* **Reference (H):** A proxy for the full wild distribution of the taxon, typically derived from herbarium specimens and observational records.

### Metrics & Scores
The application utilizes the methodology developed by Ramirez-Villegas et al. (2010) to generate three primary scores, ranging from **0 (no coverage)** to **100 (complete coverage)**.
...
* **How it works:** Buffers are drawn around distinct G and H points. The score is the proportion of the total area of all buffers covered by the G area.
...
* **How it works:** It overlays points onto a map of terrestrial ecoregions. It compares the number of ecoregions within the buffer distance of G records to the number of ecoregions containing a record.
* **Interpretation:** A high score suggests the collection includes populations adapted to the large range of environmental conditions the taxon has been observed to inhabit.
...
* **No Retention:** Any CSV file you upload is held in memory solely for the duration of your analysis.
...
### Taxonomy
The app utilizes the **GBIF Backbone Taxonomy**. When running an analysis, it is up to you to determine the best taxonomic match to your uploaded data. The application will not prevent you from upload data from different taxon, as it is understood that taxonomic nomenclature is a dynamic process.

---

## Project Background

The need for this tool arose from the growing focus understanding the **Metacollections**—combined holdings of a species maintained across multiple institutions. 

With support from the Institute of Museum and Library Services (IMLS), the Atlanta Botanical Garden led a multi-institutional team to translate an existing gap analysis code base into this web-based GAMMA interface.
...
* **Ramirez-Villegas, J. et al** (2010) A gap analysis methodology for collecting crop genepools: a case study with Phaseolus beans. *PLoS One** 5, e13497. doi:10.1371/journal.pone.0013497
